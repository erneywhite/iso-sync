...
- **Приватные каталоги**: каталог с маркером `.private` выпадает из листинга,
  тоталов, спарклайна и «Истории» (см. [docs/PRIVATE-DIRS.md](docs/PRIVATE-DIRS.md)).
  Нужно для образов Microsoft: качать их можно, перераздавать публично — нет.
  Фильтр в `index.php` — только витрина; раздачу закрывает `auth_basic` в nginx.
  Признак живёт маркером на диске, а НЕ в конфиге: битый конфиг молча отключил бы
  фильтр (fail-open на том, что прячем). Любой новый вывод имён файлов в UI
  проверять на утечку приватных имён.
