...
CI гоняет тесты на PHP 8.1–8.4 (`.github/workflows/tests.yml`) плюс `php -l`
по всем файлам (скрипты в корне тестами не запускаются).
Локально `php` может отсутствовать в PATH — тогда тесты идут на сервере / в CI.
Сборки как таковой нет (интерпретируемый PHP).

**Не использовать `mb_*` / `iconv`**: на проде CLI-PHP собран без mbstring
(прецедент: `mb_substr` в диагностическом скрипте уронил его прямо на сервере).
Расширение убрано и из CI, чтобы окружение не было щедрее боевого. Если строку
надо резать — обычный `substr` с подчисткой оборванного UTF-8 хвоста.
На сервере два PHP: системный `/usr/bin/php` (в PATH у root) и php85 у сайта —
для cron указывать полный путь явно.
...
- **Приватные каталоги** (`lib/PrivateDirs.php`, см. [docs/PRIVATE-DIRS.md](docs/PRIVATE-DIRS.md)):
  каталог с маркером `.private` выпадает из листинга, тоталов, спарклайна и
  «Истории». В маркер можно вписать allowlist IP/CIDR (v4+v6) — с этих адресов
  каталог виден и помечен бейджем `private`. Нужно для образов Microsoft: качать
  их можно, перераздавать публично — нет.
  - Фильтр в `index.php` — **только витрина**; раздачу закрывает `allow`/`deny`
