...
```bash
php update_iso.php
php update_iso.php --only=Debian_12.iso,ubuntu-lts   # только выбранные записи
```
...
**Флаг `--only=ключ1,ключ2,...`** — частичный прогон: обрабатываются только записи с указанными ключами из `config/iso-list.json` (ключ — имя ключа в объекте `files`, например `Debian_12.iso`, `ubuntu-lts`, `proxmox-ve-8`). Работает с записями любого режима: `fixed`, `latest`, `family`, `discovery`. Без флага поведение не меняется. Если ключ неизвестен (или значение пустое), скрипт ничего не скачивает и завершается кодом 2 с перечнем допустимых ключей. В `logs/last_run.json` такой прогон помечается полем `only` со списком ключей — карточка «Состояние» в веб-интерфейсе показывает, что прогон был частичным и какие записи в нём участвовали.
...
- **Состояние** — последний прогон `update_iso.php`: `актуально / N обновлено / N ошибок`, при FATAL — сообщение об ошибке. При частичном прогоне (`--only`) рядом появляется бейдж «частичный» и список записей, которые участвовали.
...
`only` — `null` для полного прогона; при запуске с `--only=...` содержит список ключей записей, которые обрабатывались (см. [обновление ISO](#update_iso-php-—-проверка-и-загрузка)).
