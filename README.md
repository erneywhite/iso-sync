...
- Современный веб-интерфейс: bento-статус из 3 карточек со sparkline-графиком роста хранилища, поиск по имени или SHA256-хэшу, **latest-бейджи по семействам**, индикатор свежести у даты файла, бренд-цвета дистрибутивов, таймлайн «История обновлений», иллюстрированный empty-state. Все анимации уважают `prefers-reduced-motion`.
...
- **Состояние** — последний прогон `update_iso.php`: `актуально / N обновлено / ошибки`, при FATAL — сообщение об ошибке.
...
**Поиск (Ctrl+K)** — по имени файла. Если в поле hex-строка длиной ≥ 6 символов — ищет и по SHA256-хэшу. Крестик справа очищает; `Esc` тоже.

**«Отсутствующие файлы»** — раскрывающийся блок с записями из конфига, которых нет на диске (с ожидаемым URL). Family/discovery записи в этот блок не попадают — для них фактическое имя становится известно только в момент `update_iso.php`.
...
**Доступность** — все интерактивные анимации (counter-up, stagger строк, sparkline draw-on, hover-glow, magnetic-pull у кнопок, 3D-tilt у bento-карточек, пульсирующие freshness-точки) выключаются при системной настройке `prefers-reduced-motion: reduce`.
