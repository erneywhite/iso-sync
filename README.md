...
Отдельно — **Windows**. Публичных зеркал для его образов не существует и не появится: лицензия не разрешает перераздачу. Поэтому они не качаются, а **собираются** из официальных пакетов обновлений, которые Microsoft раздаёт открыто — см. [Образы Windows](#образы-windows).
...
- [Образы Windows](#образы-windows)
- [Приватные каталоги](#приватные-каталоги)
...
### Образы Windows
- **Сборка из официальных пакетов Microsoft** через UUP dump — зеркал для Windows не существует, лицензия их не допускает
- Канал задаётся явно (`title_pattern`), а не «самое свежее по дате»: иначе вместо стабильной GA приезжает Insider-билд
- SHA1 каждого пакета проверяет `aria2` при загрузке; докачка после обрыва, пропуск уже собранного, ротация прошлых версий
- Windows 11, Windows Server 2025, Windows 10 — проверено на практике

### Приватные каталоги
- Маркер `.private` убирает каталог из листинга, тоталов, спарклайна и «Истории»
- **Allowlist по IP/CIDR** (v4 и v6) прямо в маркере — со своих адресов каталог виден и помечен бейджем
- `X-Forwarded-For` читается только от доверенных прокси из конфига, иначе листинг открыл бы любой подставленный заголовок
...
Отдельным механизмом — образы Windows, которые **собираются**, а не качаются:

| Образ               | Ветка         | Источник пакетов                    |
|---------------------|---------------|-------------------------------------|
| Windows 11          | 25H2 (26200)  | UUP dump → CDN Microsoft            |
| Windows Server 2025 | 26100         | UUP dump → CDN Microsoft            |
| Windows 10          | 22H2 (19045)  | UUP dump → CDN Microsoft            |

Windows Server 2022 через UUP недоступен — ветки 20348 в базе нет. Подробности — в [Образы Windows](#образы-windows).
...
│   ├── uup-builds.json       # Образы Windows для сборки
│   └── trusted-proxies.txt   # Доверенные прокси для X-Forwarded-For (по умолчанию пусто)
...
│   ├── OnlySelector.php      # разбор флага --only у update_iso.php + отбор записей
│   ├── PrivateDirs.php       # приватные каталоги + allowlist по IP/CIDR
│   ├── UupResolver.php       # выбор нужной сборки Windows среди того, что отдаёт API
│   ├── UupBuilder.php        # input для aria2, ротация, состояние сборок
│   └── Updater.php
├── docs/
│   ├── UUP-BUILDS.md         # Сборка образов Windows
│   ├── PRIVATE-DIRS.md       # Приватные каталоги и закрытие раздачи
│   └── SESSION-HANDOFF.md    # История решений проекта
...
├── .uup-work/                # Рабочий каталог сборки Windows (gitignored)
├── index.php                 # Веб-интерфейс
├── update_iso.php            # CLI: проверка и загрузка
├── generate_all_hashes.php   # CLI: пересчёт SHA256 + чистка кэша
├── build_uup.php             # CLI: сборка образов Windows
├── diag_private.php          # Диагностика приватных каталогов
├── diag_uup.php              # Диагностика готовности к сборке
...
- *(Только для сборки Windows)* тулчейн конвертера — `apt install aria2 cabextract wimtools chntpw genisoimage`, плюс ~40 ГБ свободного места на время сборки. Проверить готовность: `php diag_uup.php`
...
```bash
php update_iso.php                        # весь список
php update_iso.php --only=Debian_12.iso   # одна запись
php update_iso.php --only=Debian_12.iso,ubuntu-lts,proxmox-ve-8
php update_iso.php --help                 # краткая справка
```
...
#### Флаг `--only` — обновить выбранные записи

Без флага прогон проходит по всему списку — поведение прежнее. С флагом обрабатываются только записи с указанными ключами из `files` в `config/iso-list.json`, причём режим записи (fixed / latest / family / discovery) значения не имеет: отбор идёт по ключу до любой сетевой работы.

Формы записи флага равносильны:

```bash
php update_iso.php --only=Debian_12.iso            # ключ через запятую
php update_iso.php --only=Debian_12.iso,proxmox-ve-8
php update_iso.php --only=Debian_12.iso --only=ubuntu-lts   # повторы складываются
php update_iso.php --only Debian_12.iso            # значение отдельным аргументом
```

Правила и ограничения:

- Аргументы разбираются **до** взятия блокировки и до первого сетевого запроса: на ошибке не скачивается ничего.
- Неизвестный ключ, пустое значение (`--only=`, `--only=a,,b`) и неизвестный аргумент — сообщение в STDERR и ненулевой код выхода. При неизвестном ключе в тексте ошибки печатается список допустимых (список длинный — обрезается с счётчиком «и ещё N»); комментарийные `_comment_*` в него не попадают, их `Config` отбрасывает на парсинге.
- Повтор одного ключа и пробелы вокруг него не значимы: `--only=a.iso, a.iso ` эквивалентно `--only=a.iso`.
- Частичный прогон **не** меняет поведение по остальным записям: блокировка, чистка осиротевших `*.tmp` в начале и полный пересчёт хэшей в конце (`generate_all_hashes.php`) работают как обычно — пересчёт всё равно идёт по всему `files/`.
- В `logs/last_run.json` такой прогон дописывает поле `only` со списком ключей; веб-интерфейс в карточке «Состояние» показывает бейдж «частичный: …» (первые четыре имени, остальные счётчиком, полный список — в подсказке). Счётчики «актуально / обновлено / ошибки» в частичном прогоне относятся только к выбранным записям.

Логика отбора — `lib/OnlySelector.php` (`parseArgs` разбирает argv, `select` отбирает записи конфига), покрыта `tests/OnlySelectorTest.php`.

Код возврата:

- `0` — все файлы актуальны или успешно обновлены
- `1` — были ошибки (детали в `logs/update.log` и `logs/last_run.json`)
- `2` — фатальная ошибка (битый конфиг, нет доступа к каталогам и т.п.) либо неразобранный аргумент `--only` / неизвестный аргумент
...
### `build_uup.php` — сборка образов Windows

```bash
php build_uup.php --dry-run              # что будет собрано, без загрузки
php build_uup.php                        # собрать всё включённое в конфиге
php build_uup.php --entry=windows-11-25h2  # только одну запись
php build_uup.php --list='Windows Server'  # что вообще есть в базе UUP
```

Отдельный скрипт со своей блокировкой и расписанием: прогон идёт часами и упирается в диск и процессор, тогда как `update_iso` — быстрая проверка. Подробности — в [Образы Windows](#образы-windows).
...
- **Состояние** — последний прогон `update_iso.php`: `актуально / N обновлено / N ошибок`, при FATAL — сообщение об ошибке. Если прогон был частичным (`--only`), добавляется бейдж `частичный: <ключи>` — счётчики в таком прогоне считаются только по выбранным записям.
...
Зеркал для Windows не существует и не появится: лицензия Microsoft не разрешает третьим лицам перераздачу образов — именно поэтому их нет ни на одном нормальном зеркальном узле, а есть только на сомнительных сайтах. Зато **официальные пакеты обновлений раздаются открыто**, и из них собирается полноценный ISO. Этим занимается [`build_uup.php`](build_uup.php).

Файлы качаются не с uupdump — тот отдаёт только список. Сами пакеты идут с `tlu.dl.delivery.mp.microsoft.com`, то есть с серверов Microsoft.

```jsonc
"windows-11-25h2": {
    "enabled":       true,
    "local_subdir":  "Windows OS",
    "name_template": "Windows_11_25H2_{build}_{lang}_x64.iso",
    "title_pattern": "/^Windows 11, version 25H2 /",
    "arch":          "amd64",
    "lang":          "en-us",
...
> [!IMPORTANT]
> Канал задаётся `title_pattern`, а **не** сортировкой по дате. В базе UUP свежайшие по дате записи — preview-ветки, и «взять самое новое» тихо притащит Insider-билд вместо стабильной GA. Заметить это можно уже постфактум, на зеркале.

Заголовки в базе не всегда очевидны: Windows 10 подписана как `Feature update to Windows 10, version 22H2` — под этим именем лежит полноценный образ, а не патч. Смотреть фактические значения — через `--list`.

Надёжность: SHA1 каждого пакета проверяет `aria2` при загрузке (у собранного ISO апстримной чек-суммы не существует, поэтому порчу надо ловить на уровне исходных файлов); обрыв догружается с места остановки; готовый образ с прошлого прогона не пересобирается; место проверяется до старта; блокировка не даёт сборкам наложиться.

Полное описание — [`docs/UUP-BUILDS.md`](docs/UUP-BUILDS.md).

---

## Приватные каталоги

Если на зеркале лежит то, что не предназначено для публичной раздачи (те же образы Microsoft), каталог помечается файлом-маркером `.private` — и пропадает из листинга, тоталов, спарклайна и блока «История».

В маркер можно вписать адреса, с которых каталог всё-таки виден:
...
Поддерживаются IPv4 и IPv6, одиночные адреса и подсети. Когда каталог показан по allowlist, он помечается бейджем `private` — чтобы не спутать его с публичной раздачей.

> [!WARNING]
> Маркер — это **витрина, а не защита**. Каталог исчезает из UI, но веб-сервер продолжает отдавать файлы по прямой ссылке. Реальный доступ закрывается тем же списком IP в nginx (`allow` / `deny all`) — см. [`docs/PRIVATE-DIRS.md`](docs/PRIVATE-DIRS.md).

Клиент определяется по `REMOTE_ADDR`. `X-Forwarded-For` читается только если сам `REMOTE_ADDR` есть в [`config/trusted-proxies.txt`](config/trusted-proxies.txt) — иначе приватный листинг открывал бы любой, кто подставит заголовок. Для связки nginx + Apache (частый режим aaPanel) это как раз тот случай: PHP видит `127.0.0.1`, а настоящий адрес приходит в заголовке.
...
В частичном прогоне (`--only`) первым полем добавляется `only` со списком выбранных ключей — по нему веб-интерфейс понимает, что счётчики относятся не ко всему зеркалу. См. [`update_iso.php` → флаг `--only`](#флаг---only--обновить-выбранные-записи).
...
# Сборка образов Windows — раз в неделю, отдельной задачей
30 4 * * 0  php /path/to/build_uup.php > /dev/null 2>&1
```
...
Про сборку Windows стоит знать два момента:

- **Разносить по времени с `update_iso`.** Локи у них разные, поэтому одновременный запуск не блокируется, а вот пересчёт хэшей общий и защищён своим локом: кто добежит вторым, тихо выйдет, и свежесобранный образ останется без SHA256 до следующего прогона.
- **Еженедельно — не расточительно.** Апстрим обновляется раз в месяц, и когда сборка не изменилась, прогон заканчивается сразу после одного запроса к API, ничего не качая. Зато свежий билд подхватывается в пределах недели, а не по фиксированной дате.

Если задачи заводятся через панель (aaPanel), в [`crontab`](crontab) описано, почему вывод скрипта может не попадать в лог задачи и как это чинится через `tee` с сохранением кода возврата.
...
- `PrivateDirsTest` — CIDR-матчинг v4/v6, разбор allowlist, доверенные прокси
- `UupResolverTest` — выбор сборки: GA против preview, пакеты обновлений, сравнение ревизий
- `UupBuilderTest` — input для aria2, ротация версий, состояние сборок

Расширить можно, добавив `tests/SomethingTest.php` — runner подхватит автоматически.

CI гоняет тесты на PHP 8.1–8.4, плюс `php -l` по всем файлам и холостой прогон диагностик: тесты покрывают только `lib/`, а скрипты в корне ими не запускаются, и раньше опечатка в них всплывала уже на сервере. `mbstring` в матрицу намеренно не включён — на проде CLI-PHP собран без него, и окружение не должно быть щедрее боевого.

---

## Диагностика

Два самодостаточных скрипта — только читают, ничего не меняют. Запускаются на сервере, вывод показывает, что не так.

```bash
php diag_private.php            # приватные каталоги: маркер, allowlist, IP из логов nginx
php diag_private.php 203.0.113.42   # плюс проверка конкретного адреса
php diag_uup.php                # готовность к сборке: тулчейн, место, сеть до CDN Microsoft
```

`diag_uup.php` проверяет не «отвечает ли хост», а реальную цепочку: берёт из API настоящую ссылку на файл обновления и тянет первый мегабайт с CDN Microsoft по IPv4 и IPv6, проверяя поддержку Range и замеряя скорость.
...
- **У собранных образов Windows нет апстримной чек-суммы** — их не скачали, а собрали. Целостность гарантируется на уровне исходных пакетов (SHA1 каждого проверяет `aria2`), а SHA256 итогового ISO считается локально после сборки.
- **Пересборка определяется по имени файла**, а не по файлу состояния: номер сборки зашит в имя, поэтому потеря `logs/uup-state.json` не приводит к повторной загрузке и часу конвертации впустую.
- **Права на собранные ISO** — `0644` (данные, не программа). Переопределяется через `file_mode` в `config/uup-builds.json`.
